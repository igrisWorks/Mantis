...
async def rag_ingest_pdf(ctx: inngest.Context):
    ...
    def _upsert(chunks_and_src: RAGChunkAndSrc) -> RAGUpsertResult:
        ...
        QdrantStorage().upseert(ids, vec, payloads)
...
async def rag_query_pdf(ctx: inngest.Context):
    def _search(question: str , top_k: int = 5):
        ...
        return RAGSerachResult(contexts=found["contexts"], sources=found["sources"])
    
    question = ctx.event.data["question"]
    top_k = int( ctx.event.data.get("top_k", 5))
    found = await ctx.step.run(step_id="embed-and-serach", fn=lambda: _search(question, top_k), output_type=RAGSerachResult)
    ...
    res = await ctx.step.ai.infer(
        ...
        body ={
            ...
            "temperature": 0.2,
            "messages": [
                {"role": "system", "content": "You are the questions based on the provided context."},
